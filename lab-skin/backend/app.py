...
# ── LabOS Chat Endpoints ─────────────────────────────────────────
import sys, os
sys.path.insert(0, os.path.dirname(__file__))
from chat import process_chat, get_projects, route_to_agent

@app.route("/projects", methods=["GET"])
def get_projects_endpoint():
    """Return list of active projects from research graph."""
    projects = get_projects()
    return jsonify({"projects": projects, "ok": True})
...
def chat_endpoint():
    ...
    result = process_chat(agent, message, project, history)
    
    # Update agent state in office
    if not result.get("needs_clarification") and result.get("state"):
        agent_used = result["agent_used"]
        state = result["state"]
        # Find and update the agent's state
        with join_lock:
            agents = load_agents_state()
            for a in agents:
                if a.get("name") == agent_used:
                    a["state"] = state
                    a["detail"] = f"[chat] {message[:50]}..."
...
@app.route("/suggest-agent", methods=["POST"])
def suggest_agent():
    """Given a message, suggest which agent should handle it."""
    data = request.get_json()
    message = data.get("message","")
    suggested = route_to_agent(message)
    return jsonify({"agent": suggested, "ok": True})
